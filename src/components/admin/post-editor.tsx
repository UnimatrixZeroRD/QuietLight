...
export function PostEditor({ postId }: { postId: string }) {
  ...
  if (isLoading) {
    return <div className="lantern-panel mt-10 rounded-3xl p-8 text-[var(--muted-silver)]">Loading blog post...</div>;
  }
    ...
    <form className="lantern-panel mt-10 rounded-3xl p-8" onSubmit={handleSubmit}>
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[rgba(216,168,79,0.22)] pb-6">
        ...
        {status === "published" && slug ? (
          <Link className="gold-text text-xs uppercase tracking-[0.2em]" href={`/blog/${slug}`}>
            Open public article <span aria-hidden="true">→</span>
          </Link>
        ) : null}
      </div>

      <label className="mt-8 block text-sm uppercase tracking-[0.25em] text-[var(--soft-gold)]" htmlFor="edit-post-title">
        Title
      </label>
      <input
      ...
      <label className="mt-6 block text-sm uppercase tracking-[0.25em] text-[var(--soft-gold)]" htmlFor="edit-post-subtitle">
        Subtitle <span className="normal-case tracking-normal text-[var(--muted-silver)]">(optional)</span>
      </label>
      <input
        className="mt-4 w-full rounded-2xl border border-[rgba(216,168,79,0.4)] bg-[rgba(7,17,31,0.85)] px-5 py-4 text-[var(--ivory)] outline-none focus:border-[var(--lantern-gold)]"
        id="edit-post-subtitle"
        value={subtitle}
        onChange={(event) => setSubtitle(event.target.value)}
      />

      <label className="mt-6 block text-sm uppercase tracking-[0.25em] text-[var(--soft-gold)]" htmlFor="edit-post-slug">
        Slug
      </label>
      <input
        className="mt-4 w-full rounded-2xl border border-[rgba(216,168,79,0.4)] bg-[rgba(7,17,31,0.85)] px-5 py-4 text-[var(--ivory)] outline-none focus:border-[var(--lantern-gold)]"
      ...
      <label className="mt-6 block text-sm uppercase tracking-[0.25em] text-[var(--soft-gold)]" htmlFor="edit-post-excerpt">
        Summary
      </label>
      <textarea
        className="mt-4 min-h-28 w-full rounded-2xl border border-[rgba(216,168,79,0.4)] bg-[rgba(7,17,31,0.85)] px-5 py-4 text-[var(--ivory)] outline-none focus:border-[var(--lantern-gold)]"
      ...
      <div className="mt-6 flex flex-wrap items-end justify-between gap-3">
        <label className="block text-sm uppercase tracking-[0.25em] text-[var(--soft-gold)]" htmlFor="edit-post-body">
          Article body
        </label>
        <p className="text-xs uppercase tracking-[0.16em] text-[var(--muted-silver)]">{wordCount} words · about {readingMinutes} min read</p>
      </div>
      <textarea
        className="mt-4 min-h-[32rem] w-full rounded-2xl border border-[rgba(216,168,79,0.4)] bg-[rgba(7,17,31,0.85)] px-5 py-4 font-mono text-sm leading-7 text-[var(--ivory)] outline-none focus:border-[var(--lantern-gold)]"
        id="edit-post-body"
        value={body}
      ...
      <label className="mt-6 block text-sm uppercase tracking-[0.25em] text-[var(--soft-gold)]" htmlFor="edit-post-status">
        Publishing status
      </label>
      <select
        className="mt-4 w-full rounded-2xl border border-[rgba(216,168,79,0.4)] bg-[rgba(7,17,31,0.85)] px-5 py-4 text-[var(--ivory)] outline-none focus:border-[var(--lantern-gold)]"
      ...
      <div className="mt-7 flex flex-wrap items-center gap-4">
        ...
        <button
          className="rounded-full border border-[rgba(216,168,79,0.38)] px-6 py-3 text-sm uppercase tracking-[0.18em] text-[var(--ivory)]"
          type="button"
          onClick={loadPost}
          disabled={isSaving}
        >
